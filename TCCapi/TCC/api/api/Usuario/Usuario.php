    ...
    if ($method == "POST") {
        ...
        if($acao == "login"){
            try {
                ...
                $stmt = $db->prepare("SELECT id,ativo, senha FROM `usuarios` WHERE `email` = :email");
                $stmt->bindParam(':email', $email);
                $result = $stmt->execute();
        ...
        if($acao == "deletar"){
            try {
                ...
                $stmt = $db->prepare("SELECT id, tipo FROM usuarios WHERE id = :id");
                $stmt->bindParam(':id', $param);
                $stmt->execute();
                ...
                if($user['tipo'] === 'R'){
                    echo json_encode([
                        'status' => 'error',
                        'message' => 'Não é possível deletar um usuário pai que possui filhos cadastrados'
                    ]);
                    exit;
                }
